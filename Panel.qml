...
Panel {
  ...
  readonly property string chipKind: hasOutput ? Model.sinkKind(sink) : "none"
  readonly property real chipLevel: outputMuted ? 0 : outputVolume
  readonly property real chipActivity: {
    if (outputMuted || !hasOutput) return 0
    var peak = Number(outputPeakMonitor.peak)
  ...
  PwNodePeakMonitor {
    ...
    enabled: !!root.volumeSink && !root.outputMuted
  }
  ...
  WidgetButton {
    ...
    tooltipText: "Audio Pulse · " + (root.hasOutput ? Model.volumeReadout(root.outputVolume) + (root.outputMuted ? " muted" : "") + " · " + root.nodeLabel(root.sink) : "No output") + "\nLeft-click: mixer · Right-click: mute · Scroll: volume"
