...
function hex2(n) {
  n = Math.max(0, Math.min(255, Math.round(Number(n) || 0)))
...
function volumeReadout(volume) {
  if (volume === undefined || volume === null || isNaN(Number(volume))) return "—"
  return Math.round(clamp(volume, 0, 1) * 100) + "%"
}

function barTag(muted, kind) {
  if (muted) return "MUTE"
  if (kind === "headphones") return "CANS"
...
function sinkKind(node) {
  ...
  if (blob.indexOf("hdmi") !== -1 || blob.indexOf("display") !== -1) return "hdmi"
  return "speaker"
}
